...
def collect_preference_policy_single(env, sac_agent, n=2000, state_indices=None, render=False):
    T = sac_agent.max_ep_len
    s_buffer = np.empty((n, T+1, env.observation_space.shape[0]), dtype=np.float32)
    a_buffer = np.empty((n, T, env.action_space.shape[0]), dtype=np.float32)
    log_a_buffer = np.empty((n, T))
    
    r_buffer = np.empty((n, T, 1), dtype=np.float32)
    ...
    for traj_no in range(n):
        ...
        for i in range(T):
            a, logpi = sac_agent.get_action(s,get_logprob=True)
            s_nxt, r, _, _ = env.step(a) # assign reward online
            s_buffer[traj_no,i,:] = s
            a_buffer[traj_no,i,:] = a
            r_buffer[traj_no,i,:] = r
            log_a_buffer[traj_no,i] = logpi
            s = s_nxt
            if render:
                env.render()
    ...
    if state_indices is None:
        return s_buffer, a_buffer, log_a_buffer, r_buffer
    else:
        return s_buffer[:, :, state_indices], a_buffer, log_a_buffer,  r_buffer
