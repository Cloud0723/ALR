...
# 遍历不同的uncertainty值
for uncertainty in 0.2 0.4 0.6 0.8
do
  # 为每个uncertainty值创建对应的目录
  mkdir -p logs/uncertainty_${uncertainty}
  
  # 遍历不同的rank值
  for rank in 16 24 32 40 48 56
  do
    # 为每个uncertainty和rank组合创建对应的目录
    mkdir -p save_policy/walker_${uncertainty}_rank_${rank}
    ...
    do
      ...
      # 创建日志文件名
      log_file="logs/uncertainty_${uncertainty}/walker_seed_${seed}_rank_${rank}_RL.log"
      ...
      nohup bash -c "
        ...
        # 运行实验
        python -u ml/RL.py \
          --config configs/samples/ALR/walker2d_RL_${rank}.yml \
          --output ./save_policy/walker_${uncertainty}_rank_${rank}/seed_${seed}/model \
