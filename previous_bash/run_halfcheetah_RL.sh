...
do
  ...
  do
    ...
    mkdir -p save_policy/halfcheetah_${uncertainty}_rank_${rank}
  ...
  do
    ...
    # 创建日志文件名
      log_file="logs/uncertainty_${uncertainty}/halfcheetah_seed_${seed}_rank_${rank}_RL.log"
    ...
    nohup bash -c "
      ...
      set -e
        ...
        # 运行实验
      python -u ml/RL.py \
          --config configs/samples/ALR/halfcheetah_RL_${rank}.yml \
          --output ./save_policy/halfcheetah_${uncertainty}_rank_${rank}/seed_${seed}/model \
...
echo "Total experiment combinations: 3 uncertainties × 5 ranks × 3 seeds = 45 runs" 