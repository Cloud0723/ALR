...
class SAC:
    ...
    def __init__(self, env_fn, replay_buffer, k=1, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
            ...
            update_after=20000, update_every=50, num_test_episodes=10, max_ep_len=1000, 
            log_step_interval=None, reward_state_indices=None,
            save_freq=10, device=torch.device("cpu"), automatic_alpha_tuning=False, reinitialize=False ,sa=False,hidden_sizes=[256,256],
            **kwargs):
        """
        ...
        self.ac = actor_critic(self.env.observation_space, self.env.action_space, k,policy_hidden_sizes=hidden_sizes, add_time=add_time, device=device, **ac_kwargs)
        ...
        self.pi_scheduler = LambdaLR(self.pi_optimizer, lr_lambda=lr_lambda)
        self.q_scheduler = LambdaLR(self.q_optimizer, lr_lambda=lr_lambda)
        num_epochs=10000
        self.cosine_scheduler_pi = CosineAnnealingLR(self.pi_optimizer, T_max=num_epochs-self.warmup_steps)
        self.cosine_scheduler_q = CosineAnnealingLR(self.q_optimizer, T_max=num_epochs-self.warmup_steps)
        self.device = device
            ...
            if self.reinitialize:
                if (t * n_parallel) >= self.update_after and (t * n_parallel) % self.update_every == 0:
                    for j in range(n_parallel):
                        ...
                        _, _, log_pi = self.update(data=batch,epoch=update_count)
    ...
    def collect_data(self,num_max=20,name='halfcheetah_medium',threshold=0):
        # only called by SMM-IRL
        # Prepare for interaction with environment
        total_steps = self.steps_per_epoch * self.epochs
        start_time = time.time()
        ...
        while num<num_max:
            ...
            while True:
                ...
                if ep_len==self.max_ep_len:
                    ...
                    o, ep_len = self.env.reset(), 0
                    break
        print(np.array(actions_l).shape)
        np.save(name+'_actions.npy',actions_l)
        np.save(name+'_next_states.npy',next_states_l)
        np.save(name+'_states.npy',states_l)
        np.save(name+'_reward.npy',rewards_l)
        np.save(name+'_dones.npy',dones_l)
        np.save(name+'_lens.npy',len_l)
        ...
        for t in range(total_steps):
            ...
            if (t) >= self.update_after:
                for param_group in self.pi_optimizer.param_groups:
                    param_group['lr'] = min((t - self.update_after) / (self.update_every * 250),1) * self.lr
...
#             if t%1000==0:
#                 for param_group in self.pi_optimizer.param_groups:
#                     param_group['lr'] = min((t - self.update_after) / (self.update_every * 1000),1) * self.lr
#                     print(t,param_group['lr'])
...
#                         new_save_path=save_path[:-3]+'_'+str(round(best_eval,2))+'.pt'
                        new_save_path=save_path[:-3]+'_epoch_'+str(t//1000)+'.pt'
                        torch.save(self.ac.state_dict(),new_save_path)
                alphas.append(self.alpha.item() if self.automatic_alpha_tuning else self.alpha)
                test_rets.append(test_epret)
                log_pis.append(log_pi)
                test_time_steps.append(t+1)
