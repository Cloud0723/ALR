...
def mlp(sizes, activation, bias=[True,False,True,True,True], output_activation=nn.Identity):
    layers = []
    for j in range(len(sizes)-1):
        act = activation if j < len(sizes)-2 else output_activation
        act = nn.Identity if j==1 else act
        layers += [nn.Linear(sizes[j], sizes[j+1],bias=bias[j]), act()]
...
class SquashedGaussianMLPActor(nn.Module):
    ...
    def forward(self, obs, deterministic=False, with_logprob=True, use_std=False):
        ...
        if with_logprob:
            ...
            correction = (2*(np.log(2) - pi_action - F.softplus(-2*pi_action)))
    ...
    def log_prob(self, obs, act):
        ...
        correction = (2*(np.log(2) - act - F.softplus(-2*act)))
        if len(correction.shape) > 1:
            correction = correction.sum(axis=1)
        else:
            correction = correction.sum()
...
class MLPActorCritic(nn.Module):
    ...
    def __init__(self, observation_space, action_space, k, policy_hidden_sizes=(64,64,64),hidden_sizes=(64,64,64), add_time=False,
        ...
        if k == 1:
            self.pi = SquashedGaussianMLPActor(obs_dim, act_dim, policy_hidden_sizes, activation, act_limit).to(self.device)
        else: # we did not use GMM policy, this is experimental.
            self.pi = SquashedGmmMLPActor(obs_dim, act_dim, policy_hidden_sizes, activation, act_limit, k).to(self.device)
