...
def PIRL_loss(alpha,agent_samples, expert_samples, medexp_samples, medium_samples, reward_func, device):
    ...
    #gradient todo 
    loss = t1.mean() - t2[:1000].mean() #[:1000] means we only use one traj
...
if __name__ == "__main__":
    ...
    # assumptions
    # logs
    exp_id = f"logs/{env_name}/exp-{num_expert_trajs}-{alpha}-IRL-medexp/{v['obj']}" # task/obj/date structure
    # exp_id = 'debug'
