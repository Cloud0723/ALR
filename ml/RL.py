...
import argparse
import Non_stationary_env
from Non_stationary_env.walker2d_random import Walker2dRandomEnv, Walker2dTransferEnv
from Non_stationary_env.hopper_random import HopperRandomEnv, HopperTransferEnv
from Non_stationary_env.ant_random import AntRandomEnv, AntTransferEnv
from Non_stationary_env.humanoid_random import HumanoidRandomEnv, HumanoidTransferEnv
import random
...
parser.add_argument('--output', metavar='N',default="./save_policy/stage1", type=str,
                    help='an integer for the accumulator')
# 添加环境类型参数
parser.add_argument('--env_type', metavar='N',default="random", type=str,
                    help='environment type: random or transfer')
parser.add_argument('--uncertainty', metavar='N',default=0.5, type=float,
                    help='uncertainty factor')
...
if __name__ == "__main__":
    ...
    # 确定非平稳环境名称
    # 从原始环境名称中提取基础名称（例如从'Hopper-v3'提取'Hopper'）
    
    print(f"Using non-stationary environment: {env_name}")
    ...
    # 使用非平稳环境
    train_env = gym.make(env_name, uncertainty=args.uncertainty)
    test_env = gym.make(env_name, uncertainty=0)
