...
if __name__ == "__main__":
    ...
    # logs
    exp_id = f"logs/{env_name}/exp-{num_expert_trajs}-medexp/{v['obj']}" # task/obj/date structure
    # exp_id = 'debug'
    ...
    expert_trajs=np.load('/home/ML-IRL-main/halfcheetah_Pdata/11500/halfcheetah_medium_states.npy')[:num_expert_trajs, :, :]
    expert_samples = expert_trajs.copy().reshape(-1, len(state_indices))
    med_exp_s=np.load('/home/ML-IRL-main/halfcheetah_Pdata/10000/halfcheetah_medium_states.npy')[:num_expert_trajs, :, :]
    medexp_samples = med_exp_s.copy().reshape(-1, len(state_indices))
    medium_s=np.load('/home/ML-IRL-main/halfcheetah_Pdata/7000/halfcheetah_medium_states.npy')[:num_expert_trajs, :, :]
    medium_samples = medium_s.copy().reshape(-1, len(state_indices))
    
    
    expert_samples=medexp_samples# select
