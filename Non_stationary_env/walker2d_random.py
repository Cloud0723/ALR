...
class Walker2dRandomEnv(Walker2dEnv):
    def __init__(self, uncertainty=1, **kwargs):
        super().__init__(**kwargs)
        self.default_torso_len = 0.2  # 默认躯干长度
        self.default_foot_len = 0.1   # 默认脚长度
        self.w_torso = 0.05            # 躯干长度变化的频率
        self.w_foot = 0.05          # 脚长度变化的频率
        self.amp_torso = 0.2 * uncertainty         # 躯干长度变化的振幅
        self.amp_foot = 0.1 * uncertainty          # 脚长度变化的振幅
        self.t = 0                    # 时间步计数
    ...
    def reset(self, torso_len=None, foot_len=None):
        ...
        # print(torso_len, foot_len)
        # 确保参数在合理范围内
        torso_len = max(0.1, min(0.4, torso_len))
        foot_len = max(0.05, min(0.2, foot_len))
...
if __name__ == "__main__":
    env = gym.make("Walker2dRandom-v0")
